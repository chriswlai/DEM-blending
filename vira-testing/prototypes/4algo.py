...
def avg_point(point1, point2):
    return ((point1[0] + point2[0])/2, (point1[1] + point2[1])/2)

def find_nearest_index(main_edge, sec_edge):
    if len(main_edge) < 2 or not sec_edge:
        return -1
    
    average_point = avg_point(main_edge[0], main_edge[1])

    print("avg", average_point)

    j = sec_edge.index(min(sec_edge, key=lambda v: distance(v, average_point)))
...
def create_mesh(edge_dem2, surrounding_points_dem1):
    ...
    j = find_nearest_index(main_edge, sec_edge)
    print(sec_edge[j])
    sec_edge = sec_edge[j:] + sec_edge[0:j]
    print("closest", sec_edge[0])
    print(sec_edge)
    ...
    for i in range(main_len):
        ...
        if distance(current_vertex, sec_edge[j+1]) > distance(current_vertex, sec_edge[j]):
            closest_vertex = sec_edge[j]
        else:
            closest_vertex = sec_edge[j+1]
            j += 1
